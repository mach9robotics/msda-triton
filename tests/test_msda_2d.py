...
Test suite for Triton 2D Multi-Scale Deformable Attention kernel.
...
class TestMSDA2DTriton:
    """Test suite for Triton MSDA2D kernel correctness."""
    ...
    def _generate_test_inputs(
        ...
        """Generate test inputs for MSDA2D.
        ...
        Args:
            ...
            spatial_shapes: List of (H, W) tuples, or None for default.
        ...
        if spatial_shapes is None:
            spatial_shapes = [(32, 32), (16, 16), (8, 8), (4, 4)][:num_levels]
    ...
    def test_forward_pass_different_configs(self) -> None:
        ...
        for bs, nq, nh, dph, nl, np in configs:
            ...
            out_pytorch = msda2d_pytorch(value, spatial_shapes, sampling_locs, attn_weights)
            out_triton = msda2d_fwd(value, spatial_shapes, sampling_locs, attn_weights)
    ...
    def test_fp64_not_supported(self) -> None:
        ...
        with pytest.raises(NotImplementedError) as exc_info:
            msda2d_fwd(value, spatial_shapes, sampling_locs, attn_weights)
    ...
    def test_edge_case_boundary_sampling(self) -> None:
        ...
        sampling_locs[0, 2, 0, 0, 0, :] = torch.tensor([0.0, 1.0])
    ...
    def test_edge_case_out_of_bounds(self) -> None:
        ...
        out_triton = msda2d_fwd(value, spatial_shapes, sampling_locs, attn_weights, clamp_grid=True)
    ...
    def test_gradient_flow_end_to_end(self) -> None:
        ...
        out_pytorch = msda2d_pytorch(value_ref, spatial_shapes, sampling_locs_ref, attn_weights_ref)
        ...
        out_triton = msda2d(value_triton, spatial_shapes, sampling_locs_triton, attn_weights_triton)
    ...
    def test_single_level(self) -> None:
        ...
        value, spatial_shapes, sampling_locs, attn_weights = self._generate_test_inputs(
            num_levels=1, spatial_shapes=[(32, 32)], requires_grad=True
        )
    ...
    def test_error_on_cpu_tensors(self) -> None:
        ...
        value = torch.randn(2, 1024 + 256, 8, 32)  # CPU tensor
        spatial_shapes = torch.tensor([[32, 32], [16, 16]], dtype=torch.int32)
        sampling_locs = torch.rand(2, 50, 8, 2, 4, 2)
    ...
    def test_half_precision(self, dtype: torch.dtype) -> None:
        ...
        out = msda2d(value_g, spatial_shapes, sampling_g, attn_g)
        out.sum().backward()
        for name, grad in [
            ("value", value_g.grad),
