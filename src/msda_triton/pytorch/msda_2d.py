...
PyTorch reference implementation of 2D Multi-Scale Deformable Attention (MSDA2D).
...
def msda2d_pytorch(
    ...
    """Compute 2D Multi-Scale Deformable Attention in PyTorch.
    ...
    │                                                                 │
    │  1. Reshape value[l] → (B*H, C, H, W)                           │
    │  2. Prepare grid → (B*H, Q, P, 2)                               │
    │  3. grid_sample (bilinear) → (B*H, C, Q, P)                     │
    ...
    # Split value tensor by spatial shapes (H*W for each level)
    ...
    spatial_shapes_int = [(int(h.item()), int(w.item())) for h, w in spatial_shapes_cpu]

    for level, (h_i, w_i) in enumerate(spatial_shapes_int):
        # Reshape value for 2D grid sampling
        value_l_ = (
            value_list[level]
            .flatten(2)
            .transpose(1, 2)
            .reshape(bs * num_heads, dims_per_head, h_i, w_i)
        )
        ...
        # 2D grid sampling with bilinear interpolation
        # output: (bs*num_heads, C, Q, P)
...
class MSDA2D(nn.Module):
    """2D Multi-Scale Deformable Attention module.
    ...
    def init_weights(self) -> None:
        """Initialize module parameters with optimized 2D sampling patterns.
        ...
        # Initialize 2D sampling patterns with spatial diversity (standard from Deformable DETR)
    ...
    def forward(
        ...
        **kwargs,
        ...
        """Forward pass for 2D Multi-Scale Deformable Attention.
        ...
        Args:
            ...
            spatial_shapes: Spatial dimensions per level (num_levels, 2).
        ...
        Raises:
            ValueError: If reference_points last dimension is not 2.
        """
        ...
        # Validate 2D spatial shapes match value size
        ...
        # Normalizer for offsets: (L, 2) -> (w, h)
        ...
        for q_start in range(0, num_query, q_chunk_size):
            ...
            sampling_offsets = self.sampling_offsets(query_chunk).view(
                bs, q_len, self.num_heads, self.num_levels, self.num_points, 2
            )
